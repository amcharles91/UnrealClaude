...
//
// Stage 1-5 generators + the Final Pass for the MapBlockout pipeline. Pure algorithm: operates on
// grids/masks/structs only, no engine/editor calls. Ported faithfully from VibeUE's
// UMapBlockoutService_Stages.cpp (UMapBlockoutService::X statics -> MapBlockoutPipeline::X free
// functions). Cell-for-cell port of the host-Python reference map_designer.py; each check name
// matches a CHECK in VibeUE's map-blockout/stage-rules.md. Renderers + orchestrators are NOT here
// (see MapBlockout_Render.cpp). Determinism via FRandomStream seeded by Config.Seed is a requirement.
...
FMapBlockoutRoadNetworkResult MapBlockoutPipeline::GenerateRoads(
...
FMapBlockoutPOIResult MapBlockoutPipeline::PlacePois(
...
FMapBlockoutFieldResult MapBlockoutPipeline::PlaceFields(
	const FMapBlockoutLandcoverGrid& Grid,
	const FMapBlockoutRoadNetworkResult& Roads,
...
FMapBlockoutFoliageResult MapBlockoutPipeline::PlaceFoliage(
	const FMapBlockoutLandcoverGrid& Grid,
	const FMapBlockoutRoadNetworkResult& Roads,
	const FMapBlockoutPOIResult& Pois,
...
FMapBlockoutRailwayResult MapBlockoutPipeline::PlaceRailway(
	const FMapBlockoutLandcoverGrid& Grid,
	const FMapBlockoutRoadNetworkResult& Roads,
	const FMapBlockoutPOIResult& Pois,
	const FMapBlockoutFieldResult& Fields,
...
FMapBlockoutGateResult MapBlockoutPipeline::RunFinalPass(const FMapBlockoutState& State)
{
